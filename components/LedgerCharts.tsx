...
import { ledgerChartDayTick } from "@/lib/ledger-ui";
...
type LedgerChartsProps = {
  ...
  /** Full date of the open history day; null when no day is selected. */
...
function chartSurfaceClass(active: boolean): string {
  return `absolute inset-0 transition-opacity duration-150 ease-out motion-reduce:transition-none ${active ? "opacity-100" : "pointer-events-none opacity-0"}`;
}
...
// tooltip spells out goal progress. Both stay mounted so switching is an
// interruptible opacity transition instead of an SVG teardown and replay.
// Kept out of SSR so the board stays stable.
export function LedgerCharts({ mode, dailyRows, weeklyRows, selectedDay, reduceMotion, intro }: LedgerChartsProps) {
  const weeklyActive = mode === "weekly";

  return (
    <div className="relative h-[230px]">
      <div
        className={chartSurfaceClass(weeklyActive)}
        aria-hidden={!weeklyActive}
        inert={weeklyActive ? undefined : true}
      >
                ...
                return (
                  <div className="rounded-2xl border border-white/[0.08] bg-zinc-950/96 px-3 py-2 text-xs shadow-2xl backdrop-blur">
                    ...
                    <div className="mt-2 space-y-1 text-zinc-400">
                      <div className="flex justify-between gap-6"><span>Activity score</span><span className="font-mono text-zinc-100">{week.weeklyValue}</span></div>
      ...
      <div
        className={chartSurfaceClass(!weeklyActive)}
        aria-hidden={weeklyActive}
        inert={weeklyActive ? true : undefined}
      >
        <ResponsiveContainer minHeight={230} minWidth={0} initialDimension={{ width: 600, height: 230 }}>
          <LineChart data={dailyRows} margin={chartMargin}>
            ...
            <XAxis
              dataKey="day"
              tickFormatter={ledgerChartDayTick}
              axisLine={false}
              tickLine={false}
              minTickGap={24}
              tick={axisTick}
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-2xl border border-white/[0.08] bg-zinc-950/96 px-3 py-2 text-xs shadow-2xl backdrop-blur">
                    ...
                    <div className="mt-2 space-y-1 text-zinc-400">
                      <div className="flex justify-between gap-6"><span>Activity score</span><span className="font-mono text-zinc-100">{point.dailyValue}</span></div>
