...
import {
  frameStripRange,
  frameStripStartForSelection,
  shiftFrameStripWindow,
} from "@/lib/frame-strip";
...
const BarChart = dynamic(
  () => import("@/components/charts/BarChart").then((mod) => mod.BarChart),
  {
    ssr: false,
    loading: () => (
      <div className="h-56 w-full animate-pulse rounded-md bg-white/[0.03]" />
    ),
  },
);
...
type OwnerAuthState = "idle" | "saving" | "error";
...
function resolveFramePosition(frameIndex: number, frameCount: number): number {
  return frameCount === 0
    ? -1
    : frameIndex >= 0
      ? Math.min(frameIndex, frameCount - 1)
      : frameCount - 1;
}
...
function todayMetrics(
  today: TodayStats,
  previous: TodayStats | null,
  deltaNote: string,
): MetricCard[] {
  return [
    {
      label: "Hours present",
      value: today.hoursPresent,
      decimals: 1,
      suffix: "",
      ...trend(
        today.hoursPresent,
        previous?.hoursPresent ?? null,
        1,
        deltaNote,
      ),
    },
    {
      label: "Avg focus",
      value: today.avgScore,
      decimals: 0,
      suffix: "%",
      ...trend(today.avgScore, previous?.avgScore ?? null, 0, deltaNote, "%"),
    },
    {
      label: "Headphones",
      value: today.headphonesPct,
      decimals: 0,
      suffix: "%",
      ...trend(
        today.headphonesPct,
        previous?.headphonesPct ?? null,
        0,
        deltaNote,
        "%",
      ),
    },
    {
      ...
      value: today.criticalHours,
      decimals: 0,
      suffix: "h",
      ...trend(
        today.criticalHours,
        previous?.criticalHours ?? null,
        0,
        deltaNote,
      ),
    },
...
function averageMetrics(
  ...
  return [
    {
      label: "Hours present",
      value: last7.hoursPresent,
      decimals: 1,
      suffix: "",
      ...trend(last7.hoursPresent, baseline?.hoursPresent ?? null, 1, note),
    },
    {
      label: "Avg focus",
      value: last7.avgScore,
      decimals: 0,
      suffix: "%",
      ...trend(last7.avgScore, baseline?.avgScore ?? null, 0, note, "%"),
    },
    {
      label: "Headphones",
      value: last7.headphonesPct,
      decimals: 0,
      suffix: "%",
      ...trend(
        last7.headphonesPct,
        baseline?.headphonesPct ?? null,
        0,
        note,
        "%",
      ),
    ...
    {
      label: "Critical hours",
      value: last7.criticalHours,
      decimals: 1,
      suffix: "h",
      ...trend(last7.criticalHours, baseline?.criticalHours ?? null, 1, note),
    },
  ];
}
...
export function Dashboard({
  ...
  const [frameStripVisibleCount, setFrameStripVisibleCount] = useState(
    FRAME_STRIP_FALLBACK_VISIBLE_COUNT,
  );
  const [armed, setArmed] = useState<Armed | null>(null);
  const [frameStripDirection, setFrameStripDirection] =
    useState<FrameStripDirection>(0);
  ...
  const [signalPatch, setSignalPatch] = useState<{
    id: string;
    signals: Signals;
  } | null>(null);
  const [writeError, setWriteError] = useState<string | null>(null);
  const [ownerSecretDraft, setOwnerSecretDraft] = useState("");
  const [ownerAuthState, setOwnerAuthState] = useState<OwnerAuthState>("idle");
  const [ownerSessionRequired, setOwnerSessionRequired] = useState(false);
  const [criticalPatch, setCriticalPatch] = useState<{
    day: string;
    hour: number;
    critical: boolean;
  } | null>(null);
  ...
  const selectableDays = useMemo(
    () => new Set([...data.dataDays, data.today]),
    [data.dataDays, data.today],
  );
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const nextVisibleCount = Math.max(
        1,
        Math.floor(
          (width + FRAME_STRIP_GAP_PX) /
            (FRAME_STRIP_THUMB_WIDTH_PX + FRAME_STRIP_GAP_PX),
        ),
      );
      setFrameStripVisibleCount((current) =>
  ...
  useEffect(() => {
    ...
    async function refreshFromStatus(): Promise<void> {
      ...
      checking = true;
      try {
        const res = await fetch(`${mirror ? "/mirror-api/status" : "/api/status"}?t=${Date.now()}`, {
          cache: "no-store",
        });
        if (!res.ok) return;
  ...
  }, [data.isToday, data.settings.paused, data.latest?.id, router, mirror]);

  const activeHour = selectedHour ?? data.defaultHour;
  const activeFrames =
    activeHour === null ? [] : (data.hourlyFrames[activeHour] ?? []);
  const followingLatest =
    data.isToday && frameIndex < 0 && activeHour === data.defaultHour;
  const selectedFramePosition = resolveFramePosition(
    frameIndex,
    activeFrames.length,
  );
  const selected =
    followingLatest && data.latest
      ...
      : selectedFramePosition >= 0
        ? (activeFrames[selectedFramePosition] ?? null)
        : null;
  ...
  useEffect(() => {
    ...
    setFrameStripStart(
      (current) =>
        frameStripRange(activeFrames.length, current, frameStripVisibleCount)
          .start,
    );
  }, [
    activeFrames.length,
  ...
  const { start: visibleFrameStart, end: visibleFrameEnd } = frameStripRange(
    activeFrames.length,
    frameStripStart,
    frameStripVisibleCount,
  );
  ...
  const activeCheckin =
    activeHour === null
      ? null
      : (data.hourly.find((checkin) => checkin.hour === activeHour) ?? null);
  const stepFrameStrip = useCallback(
    (delta: -1 | 1) => {
      setFrameStripDirection(delta);
      setFrameStripStart((current) =>
        shiftFrameStripWindow(
          activeFrames.length,
          current,
          frameStripVisibleCount,
  ...
  const chartData = useMemo(() => {
    const byHour = new Map(
      data.hourly.map((checkin) => [checkin.hour, checkin]),
    );
    const rows: Array<{
      hour: number;
      label: string;
  ...
  const incompleteHours = useMemo(
    () => chartData.filter((row) => row.incomplete).length,
    [chartData],
  );
  ...
  const liveFrameId =
    ...
    !data.statusState.stale &&
    data.latest
      ? data.latest.id
      : null;
  // Never "live" on a saved copy: nothing is arriving, and the frame it would
  // label is one the copy does not even carry.
  const isLive =
    !mirror &&
    selected !== null &&
    liveFrameId !== null &&
    selected.id === liveFrameId;
  const detailAvg =
    activeCheckin?.avgScore ??
    (activeFrames.length > 0
      ? Math.round(
          activeFrames.reduce((total, frame) => total + frame.score, 0) /
            activeFrames.length,
        )
      : 0);
  const detailPresentPct =
    activeCheckin?.presentPct ??
    (activeFrames.length > 0
      ? Math.round(
          (activeFrames.filter((frame) => frame.present).length /
            activeFrames.length) *
            100,
        )
      : 0);
  const detailHeadphonesPct =
    ...
    (activeFrames.length > 0
      ? Math.round(
          (activeFrames.filter(
            (frame) => frame.visionRead !== "unknown" && frame.headphones,
          ).length /
            Math.max(
              1,
              activeFrames.filter((frame) => frame.visionRead !== "unknown")
                .length,
            )) *
            100,
        )
  ...
  const detailHeadphonesUnknown =
    activeFrames.length > 0 &&
    activeFrames.every((frame) => frame.visionRead === "unknown");
  ...
  const metrics =
    tab === "today"
      ? todayMetrics(
          data.stats,
          data.previousStats,
          data.isToday ? "vs yesterday at this time" : "vs yesterday",
        )
      : averageMetrics(data.averages.last7, data.averages.previous7);
  ...
  const selectedSignals = effectiveSelected
    ? editableSignalsFor(effectiveSelected)
    : [];
  const heroStatus = effectiveSelected
    ? statusLabels[effectiveSelected.status]
    : data.statusState.headline;
  const effectiveCritical =
    criticalPatch &&
    activeHour !== null &&
    criticalPatch.day === data.viewDay &&
    criticalPatch.hour === activeHour
  ...
  async function persistCorrection(
    ...
    try {
      const response = await fetch(
        mirror ? "/mirror-api/feedback" : "/api/feedback",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ snapshotId, field, value, day: data.viewDay }),
        },
      );
      ...
      setWriteError(
        sessionRequired
          ? "Unlock editing below, then tap the correction again."
          : "Could not save that correction. Tap again.",
      );
    }
    router.refresh();
  }

  async function unlockOwnerEditing() {
    if (!ownerSecretDraft) {
      setOwnerAuthState("error");
      return;
    }
    setOwnerAuthState("saving");
    try {
      const response = await fetch(
        mirror ? "/mirror-api/ledger/session" : "/api/ledger/session",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ secret: ownerSecretDraft }),
        },
      );
      if (!response.ok) {
        throw new Error("Owner session was not accepted");
      }
      setOwnerSecretDraft("");
      setOwnerAuthState("idle");
      setOwnerSessionRequired(false);
      setWriteError("Editing unlocked. Tap the correction again.");
    } catch {
      setOwnerAuthState("error");
    }
  }
  ...
  async function persistCritical(day: string, hour: number, critical: boolean) {
    try {
      const response = await fetch(
        mirror ? "/mirror-api/critical" : "/api/critical",
        {
          method: "POST",
          headers: { "content-type": "application/json" },
      ...
      setCriticalPatch((patch) =>
        patch && patch.day === day && patch.hour === hour ? null : patch,
      );
    ...
    <main className="mx-auto max-w-5xl px-5 py-6">
        ...
        >
          ...
          <div className="min-w-0 text-sm leading-5">
            ...
            <p className="mt-0.5 text-amber-200/70">
              ...
              {data.vision.since
                ? ` · unread since ${frameTime(data.vision.since, data.timeZone)}`
                : ""}
              ...
              {data.vision.status === "credits"
                ? " · top up your AI account to turn them back on"
                : ""}
            </p>
          </div>
        </div>
      ...
      <header className="mb-6 flex items-center justify-between gap-4">
          ...
          <Popover open={dayPickerOpen} onOpenChange={setDayPickerOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                size="xs"
                className="gap-1.5 px-1.5 text-sm font-normal text-zinc-500 hover:text-zinc-100"
              >
                <CalendarDays aria-hidden />
                <span className="truncate">
                  {data.isToday
                    ? `Today · ${data.viewDayLabel}`
                    : data.viewDayLabel}
                </span>
              </Button>
            </PopoverTrigger>
        ...
        <div className="flex shrink-0 items-center gap-2 text-xs text-zinc-500">
          ...
          {/* Frozen at publish time on a copy, so it would insist "0 min ago"
              forever. The banner already states how old the copy is. */}
          {mirror ? null : (
            <>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.05fr)_minmax(300px,0.95fr)] lg:items-start">
        {/* min-w-0: a grid item defaults to min-width:auto, so the fixed-width
            filmstrip inside sets this column's floor at 440px and pushes the
            whole page into a sideways scroll on a phone. */}
        <div ref={topPanelRef} className="min-w-0">
          <div className="relative aspect-[4/3] max-h-[520px] w-full overflow-hidden rounded-2xl outline outline-1 -outline-offset-1 outline-white/10">
            {selected?.thumbUrl ? (
              ...
              <div className="flex h-full flex-col items-center justify-center px-6 text-center">
                ...
                <p className="mt-2 max-w-52 text-xs leading-5 text-zinc-500">
                  The frame keeps its place even when capture is quiet, so the
            ...
            {isLive ? (
              <span className="absolute top-2.5 left-2.5 inline-flex items-center gap-1.5 rounded-md bg-black/40 px-2 py-1 text-[10px] font-medium tracking-wide text-zinc-200 backdrop-blur">
                <span
                  className="size-1.5 rounded-full bg-zinc-100"
                  aria-hidden
                />{" "}
                LIVE
              </span>
          ...
          {activeFrames.length > 0 ? (
            <div className="mt-3 space-y-2">
                      ...
                      <motion.div
                        ...
                        initial={
                          reduceMotion
                            ...
                            : {
                                opacity: 0.84,
                                x: stripOffset(frameStripDirection, 18),
                              }
                        }
                        ...
                        {visibleFrames.map((frame, offset) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              />
                              <span
                                className="absolute inset-x-0 bottom-0 h-1.5 bg-zinc-300"
                                aria-hidden
                              />
              ...
              <div className="flex items-center justify-between text-[11px] text-zinc-500">
                <span>
                  Snapshot <Num>{String(selectedFramePosition + 1)}</Num> of{" "}
                  <Num>{String(activeFrames.length)}</Num>
                </span>
                    ...
                    <motion.span
                      ...
                      initial={
                        reduceMotion
                          ...
                          : {
                              opacity: 0,
                              x: stripOffset(frameStripDirection, 10),
                            }
                      }
                      animate={{ opacity: 1, x: 0 }}
                      exit={
        ...
        <div className="space-y-6">
          <div className="min-w-0">
            ...
            <div className="flex items-end gap-6 sm:gap-8">
              <div className="min-w-0">
                ...
                <div className="flex items-end gap-1.5">
                  <Num className="text-6xl leading-none font-semibold tracking-[-0.05em] text-zinc-50">
                    {String(effectiveSelected?.score ?? 0)}
                  </Num>
                  <span className="pb-1 text-lg text-zinc-500">/100</span>
                </div>
                <p className="mt-2 text-sm text-zinc-500">
                  {selected
                    ? frameTime(selected.capturedAt, data.timeZone)
                    : data.statusState.lastCheckedText}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="flex items-end gap-1.5">
                  <Num className="text-3xl leading-none font-semibold tracking-[-0.04em] text-zinc-200">
                    {String(detailAvg)}
                  </Num>
            ...
            <p className="mt-3 text-xs text-zinc-500">
              <Num>{String(activeFrames.length)}</Num>{" "}
              {activeFrames.length === 1 ? "snapshot" : "snapshots"} this hour
            </p>
            ...
            {activeCheckin ? (
              <motion.button
                ...
                animate={
                  reduceMotion
                    ? undefined
                    : effectiveCritical && !criticalArmed
                      ? { boxShadow: "0 0 20px rgba(69, 217, 119, 0.3)" }
                      : {}
                }
                className={cn(
                    ...
                    return (
                      ...
                      >
                        <Icon
                          className={cn(
                            "size-3",
                            isArmed ? "text-zinc-950" : "text-zinc-500",
                          )}
                          aria-hidden
                        />
                        <span className="grid">
                ...
                {writeError ? (
                  <p
                    className={cn(
                      "mt-4 text-sm",
                      writeError ===
                        "Editing unlocked. Tap the correction again."
                        ? "text-emerald-300"
                        : "text-rose-300",
                    )}
                  >
                ...
                {ownerSessionRequired ? (
                  <div className="mt-4 border-t border-white/[0.06] pt-4">
                    <p className="text-xs text-zinc-400">
                      Owner access is required to save corrections.
                    </p>
                    <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                      <input
                        aria-label="Owner secret"
                        className="h-9 rounded-md border border-white/[0.1] bg-black/30 px-3 text-sm text-zinc-100 outline-none transition focus:border-white/30 focus:ring-2 focus:ring-white/10 sm:max-w-xs"
                        type="password"
                        autoComplete="current-password"
                        placeholder="Owner secret"
                        value={ownerSecretDraft}
                        onChange={(event) =>
                          setOwnerSecretDraft(event.target.value)
                        }
                        onKeyDown={(event) => {
                          if (event.key === "Enter") {
                            event.preventDefault();
                            void unlockOwnerEditing();
                          }
                        }}
                      />
                      <Button
                        ...
                        size="sm"
                        variant="outline"
                        disabled={ownerAuthState === "saving"}
                        onClick={() => void unlockOwnerEditing()}
                      >
                        {ownerAuthState === "saving"
                          ? "Unlocking…"
                          : "Unlock editing"}
                      </Button>
                    </div>
                    {ownerAuthState === "error" ? (
                ...
                {(["today", "average"] as const).map((value) => (
                  <motion.button
                    ...
                    className={cn(
                      ...
                      tab === value ? "text-black" : "text-zinc-500",
                    )}
                  >
                    {tab === value && (
                      <motion.span
                        layoutId={
                          reduceMotion ? undefined : "dashboard-tab-active"
                        }
      ...
      <div className="mt-6">
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold">
              {data.isToday ? "Today" : data.viewDayLabel}
            </h3>
          </div>
        ...
        {incompleteHours > 0 ? (
          <p className="mt-2 text-xs leading-5 text-amber-200/70">
            ...
            {incompleteHours === 1
              ? "1 hour is"
              : `${incompleteHours} hours are`}{" "}
            missing headphone data — nothing examined those frames, so the bar
            is lower than the hour really was. Open the hour and set headphones
            to correct it.
          </p>
        ) : null}
      </div>
