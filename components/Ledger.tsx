...
const LedgerCharts = dynamic(
  () => import("@/components/LedgerCharts").then((mod) => mod.LedgerCharts),
  {
    ssr: false,
...
export type LedgerProps = {
  data: LedgerData;
  /** Saved-copy context. Its presence selects the durable Vercel mutation path. */
...
export function Ledger({ data, mirror }: LedgerProps) {
  const router = useRouter();
  const [, startTransition] = useTransition();
  const prefersReducedMotion = useReducedMotion() ?? false;
  const [hydrated, setHydrated] = useState(false);
  // Reduced motion only after hydration, so the first client render matches the
  // server's (which can't read the media query), avoiding React #418.
  ...
  useEffect(() => {
    return () => {
      Object.values(reachoutTimers.current).forEach((timer) =>
        window.clearTimeout(timer),
      );
      Object.values(repliesTimers.current).forEach((timer) =>
        window.clearTimeout(timer),
      );
      Object.values(meetingsTimers.current).forEach((timer) =>
        window.clearTimeout(timer),
      );
    ...
    () =>
      data.days.map((day) => {
        ...
        return {
          ...
          dailyValue: day.inRange
            ? dailyValue(nextReachouts, day.hours, day.targets)
            : 0,
          active:
            day.inRange &&
            (nextReachouts > 0 || day.hours > 0),
        };
      }),
    [data.days, meetingsDraft, reachoutsDraft, repliesDraft],
  );

  const viewDayMap = useMemo(
    () => new Map(viewDays.map((day) => [day.day, day])),
    [viewDays],
  );
  ...
  const viewWeeks = useMemo(
    () =>
      data.weeks.map((week) => {
        ...
        const hours =
          Math.round(elapsed.reduce((sum, day) => sum + day.hours, 0) * 10) /
          10;
        ...
        const progress = weeklyProgress(reachouts, hours, {
  ...
  const weekdayAverages = useMemo(() => {
    ...
    return WEEKDAY_COLUMNS.map((weekday, index) => ({
      weekday,
      averageValue:
        counts[index] > 0 ? Math.round(sums[index] / counts[index]) : 0,
    }));
  }, [viewDays]);
        ...
        index >= 6
          ? Math.round(
              elapsed
                ...
                .reduce((sum, item) => sum + item.dailyValue, 0) / 7,
            )
          : null,
    }));
  }, [viewDays]);
  ...
  const currentWeek =
    viewWeeks.find((week) => week.days.some((day) => day.day === data.today)) ??
    viewWeeks[viewWeeks.length - 1] ??
    null;
  const summaryWeek = currentWeek;
  const selectedReachouts = selectedDayData?.reachouts ?? 0;
  const reachoutSaveState = selectedDayData
    ? (reachoutsState[selectedDayData.day] ?? "idle")
    : "idle";
  const selectedReplies = selectedDayData?.replies ?? 0;
  const selectedMeetings = selectedDayData?.meetings ?? 0;
  const repliesSaveState = selectedDayData
    ? (repliesState[selectedDayData.day] ?? "idle")
    : "idle";
  const meetingsSaveState = selectedDayData
    ? (meetingsState[selectedDayData.day] ?? "idle")
    : "idle";
  ...
  async function persistLedgerUpdate(
    day: string,
    body: {
      reachouts?: number;
      replies?: number;
    ...
    const response = await fetch(mirror ? "/mirror-api/ledger" : "/api/ledger", {
      method: "POST",
  ...
  async function authenticateOwner(secret: string) {
    ...
    if (!response.ok) {
      throw new Error(response.status === 429 ? "AUTH_RATE_LIMITED" : "AUTH_REJECTED");
    }
    setSessionRequired(false);
  }

  async function persistWeeklyGoal(
    weekStart: string,
    reachouts: number,
    hours: number,
  ) {
  ...
  function celebrateIfPerfect(
    ...
    // A perfect day is perfect against its own week's bar, so the targets come
    // from the day itself. An unknown day key cannot be scored, so it cannot
    // celebrate either.
    const targets = viewDayMap.get(day)?.targets;
    if (
      !targets ||
      dailyValue(reachouts, hours, targets) !== 100
    ) {
      return;
    }
    if (breatheTimer.current !== null) {
  ...
  function scheduleReachoutsSave(day: string, value: number) {
    ...
    reachoutTimers.current[day] = window.setTimeout(async () => {
      try {
        ...
        celebrateIfPerfect(day, value, hours);
  ...
  function handleReachoutsChange(nextValue: number) {
    ...
    setReachoutsState((state) => ({
      ...state,
      [selectedDayData.day]: "saving",
    }));
  ...
  async function handleReachoutsRetry() {
    ...
    try {
      await persistLedgerUpdate(selectedDayData.day, {
      ...
      setReachoutsState((state) => ({
        ...state,
        [selectedDayData.day]: "idle",
      }));
    } catch {
      setReachoutsState((state) => ({
        ...state,
        [selectedDayData.day]: "error",
      }));
    }
  }
  ...
  const countHooks = {
    replies: {
      setDraft: setRepliesDraft,
      setSave: setRepliesState,
      timers: repliesTimers,
    },
    meetings: {
      setDraft: setMeetingsDraft,
      setSave: setMeetingsState,
      timers: meetingsTimers,
    },
  } as const;

  function scheduleCountSave(
    field: "replies" | "meetings",
    day: string,
    value: number,
  ) {
    ...
    timers.current[day] = window.setTimeout(async () => {
      try {
        await persistLedgerUpdate(
          day,
          field === "replies" ? { replies: value } : { meetings: value },
        );
  ...
  const summaryStats = summaryWeek
    ? [
        {
          label: "Messages",
          value: summaryWeek.reachouts,
          target: summaryWeek.reachoutsTarget,
          pct: summaryWeek.reachoutsPct,
          accent: "from-blue-400/80 to-sky-300/90",
        },
        {
          label: "Hours",
          value: summaryWeek.hours,
          target: summaryWeek.hoursTarget,
          pct: summaryWeek.hoursPct,
          accent: "from-teal-400/80 to-emerald-300/90",
        },
      ]
    : [];
  ...
  const dayReportPanel = selectedDayData ? (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      <div className="flex shrink-0 items-center justify-between border-b border-white/[0.06] px-4 py-3">
        <div className="min-w-0">
          <p className="text-[11px] uppercase tracking-[0.14em] text-zinc-500">
            {selectedDayData.weekdayLabel} · {selectedDayData.label}
          </p>
          <div className="mt-1 flex items-end gap-2">
            <span className="text-xs text-zinc-500">Activity score</span>
            <Num className="text-3xl font-semibold leading-none text-zinc-50">
              {String(selectedDayData.dailyValue)}
            </Num>
          </div>
        </div>
        <button
          ...
          className="inline-flex size-10 items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.03] text-zinc-400 transition-colors hover:border-white/[0.14] hover:text-zinc-100 active:scale-[0.96]"
      ...
      <div
        data-day-report-scroll
        className="min-h-0 flex-1 touch-pan-y overflow-y-auto overscroll-contain px-4 py-4 [scrollbar-gutter:stable]"
      >
        <LedgerDayReport
          day={selectedDayData}
          reachouts={selectedReachouts}
          ...
          dailyReachoutTarget={selectedDayData.targets.reachouts}
          dailyHoursTarget={selectedDayData.targets.hours}
        />
      </div>
    </div>
  ) : null;
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-5">
      <div className="space-y-4">
        ...
        {summaryWeek ? (
          <section className="rounded-3xl border border-white/[0.08] bg-white/[0.03] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.25)] sm:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <p className="text-[11px] uppercase tracking-[0.14em] text-zinc-500">
                  Week of {summaryWeek.label}
                </p>
                <p className="mt-1 text-xs text-zinc-500">
                  <Num animate={false}>{String(data.totals.activeDays)}</Num>{" "}
                  active days ·{" "}
                  <Num animate={false}>
                    {String(data.totals.activeDayStreak)}
                  </Num>{" "}
                  day streak
                </p>
              </div>
              <div className="flex items-center gap-2 rounded-full border border-white/[0.08] bg-black/20 px-3 py-1.5">
                ...
                <Num className="text-lg font-semibold text-zinc-50">
                  {String(summaryWeek.weeklyValue)}
                </Num>
              </div>
            </div>
            ...
            <p className="mt-4 text-xs text-zinc-400">
              ...
              <span className="text-zinc-200">
                {Math.round(summaryWeek.replyRate * 100)}%
              </span>{" "}
              · <span className="text-zinc-200">{summaryWeek.meetings}</span>{" "}
              meetings (
              <span className="text-zinc-200">
                {Math.round(summaryWeek.bookingRate * 100)}%
              </span>{" "}
              of replies) ·{" "}
              <span className="text-zinc-200">{summaryWeek.commits}</span>{" "}
              commits ·{" "}
              <span className="text-zinc-200">{summaryWeek.merges}</span> merges
            </p>
          </section>
        ) : null}
        {summaryWeek ? (
        ...
        <section className="rounded-3xl border border-white/[0.08] bg-white/[0.03] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.25)] sm:p-5">
          <div className="mb-3 flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold text-zinc-100">Activity trend</h2>
              ...
              <div className="flex items-center gap-1 rounded-full bg-black/20 p-0.5">
                {(["daily", "weekly"] as const).map((mode) => (
                  <button
                    ...
                    className={cn(
                      "relative min-h-10 rounded-full px-3 text-xs capitalize transition-[color,scale] duration-150 ease-out active:scale-[0.96]",
                      chartMode === mode
                        ? "text-black"
                  ...
                  >
                    {chartMode === mode ? (
                      <motion.span
                        layoutId={
                          reduceMotion ? undefined : "ledger-trend-pill"
                        }
          ...
          <LedgerCharts
            ...
            selectedDay={trayOpen ? selectedDay : null}
              ...
              >
                <div className="relative h-24 w-full overflow-hidden rounded-xl bg-white/[0.04]">
                  <motion.div
                    ...
                    animate={{
                      height: `${Math.max(3, average.averageValue)}%`,
                    }}
                    transition={motionTransition(reduceMotion, {
                      ...appSpring,
                      delay: intro ? index * 0.04 : 0,
                    })}
                  />
                </div>
                ...
                <div className="text-sm font-medium text-zinc-100">
                  <Num>{String(average.averageValue)}</Num>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="rounded-3xl border border-white/[0.08] bg-white/[0.03] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.25)] sm:p-5">
          ...
          <div className="grid grid-cols-[28px_repeat(7,minmax(0,1fr))] gap-1">
            ...
            {WEEKDAY_COLUMNS.map((weekday) => (
              <div
                key={weekday}
                className="pb-1 text-center text-[10px] uppercase tracking-[0.08em] text-zinc-500"
              >
                {weekday}
              </div>
            ))}
            {viewWeeks.map((week, weekIndex) => {
              ...
              const prevRailMonth =
                weekIndex > 0
                  ? viewWeeks[weekIndex - 1].days[0].monthKey
                  : null;
              ...
              return (
                <Fragment key={week.weekStart}>
                  ...
                  {week.days.map((day, columnIndex) => {
                    ...
                    const aboveMonth =
                      weekIndex > 0
                        ? viewWeeks[weekIndex - 1].days[columnIndex]?.monthKey
                        : undefined;
                    const belowMonth =
                      weekIndex < viewWeeks.length - 1
                        ? viewWeeks[weekIndex + 1].days[columnIndex]?.monthKey
                        : undefined;
                    const leftMonth =
                      columnIndex > 0
                        ? week.days[columnIndex - 1].monthKey
                        : undefined;
                    const rightMonth =
                      columnIndex < 6
                        ? week.days[columnIndex + 1].monthKey
                    ...
                    const monthTop =
                      day.inRange &&
                      aboveMonth !== undefined &&
                      aboveMonth !== day.monthKey;
                    const monthBottom =
                      day.inRange &&
                      belowMonth !== undefined &&
                      belowMonth !== day.monthKey;
                    const monthLeft =
                      day.inRange &&
                      leftMonth !== undefined &&
                      leftMonth !== day.monthKey;
                    const monthRight =
                      day.inRange &&
                      rightMonth !== undefined &&
                      rightMonth !== day.monthKey;
                    const valueFace = (
                      <div className="flex h-full flex-col justify-between p-1.5">
                        <div className="flex items-start justify-between">
                          <span className="text-[9px] font-medium text-zinc-400">
                            {day.inRange ? day.dayOfMonth : ""}
                          </span>
                        </div>
                        {day.inRange ? (
                          <Num className="self-end text-base font-semibold leading-none">
                            {String(day.dailyValue)}
                          </Num>
                        ) : null}
                      </div>
                    );
                    ...
                    return (
                      <motion.button
                        ...
                        aria-label={
                          day.inRange ? ledgerDayAriaLabel(day) : undefined
                        }
                        aria-pressed={isSelected}
                        onClick={() =>
                          day.inRange && beginDaySelection(day.day)
                        }
                        whileTap={
                          reduceMotion || !day.inRange
                            ? undefined
                            : { scale: 0.96 }
                        }
      ...
      <Sheet
        open={trayOpen && Boolean(selectedDayData)}
          ...
          <SheetContent
            ...
            className={cn(
              ...
              isWide
                ...
                : "h-[min(76vh,620px)] w-full rounded-t-[28px] rounded-b-none",
            )}
          >
            <motion.div
              initial={
