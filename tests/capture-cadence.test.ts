...
import {
  captureCadenceFor,
  shouldStoreCaptureResult,
  skippedSnapshotsForCaptureCadence,
...
test("captureCadenceFor backs off after thirty consecutive away minutes", () => {
  ...
  expect(
    captureCadenceFor(
      latest,
      [firstAway, latest],
      new Date("2026-06-20T12:35:00.000Z"),
    ),
  ).toMatchObject({
    due: false,
    intervalMinutes: 15,
...
test("captureCadenceFor resets the away streak at the morning capture start", () => {
  ...
  expect(
    captureCadenceFor(latest, [latest], new Date("2026-06-20T08:05:00.000Z")),
  ).toEqual({
    due: true,
    intervalMinutes: 5,
    awayMinutes: 5,
...
test("captureCadenceFor still backs off for a streak accumulated within the same morning", () => {
  ...
  expect(
    captureCadenceFor(
      latest,
      [firstAway, middleAway, latest],
      new Date("2026-06-20T09:35:00.000Z"),
    ),
  ).toMatchObject({
    intervalMinutes: 30,
    awayMinutes: 75,
...
test("status route reports AFK backoff state", async () => {
  ...
  const response = await getStatus(
    new Request("https://tally.test/api/status"),
  );
...
test("skippedSnapshotsForCaptureCadence thins historical away runs", () => {
  const snapshots = Array.from({ length: 13 }, (_, index) =>
    snapshot(
      `away-${index}`,
      new Date(Date.UTC(2026, 5, 20, 12, index * 5)).toISOString(),
      "away",
    ),
  );

  expect(
    skippedSnapshotsForCaptureCadence(snapshots).map((row) => row.capturedAt),
  ).toEqual([
...
test("purge AFK overflow deletes redundant away snapshots and rebuilds affected hours", async () => {
  for (let index = 0; index < 13; index += 1) {
    await saveStoredSnapshot(
      new Date(Date.UTC(2026, 5, 20, 12, index * 5)),
      "away",
    );
  }
  ...
  expect(
    (await snapshotsForDay("2026-06-20")).map((row) => row.capturedAt),
  ).toEqual([
